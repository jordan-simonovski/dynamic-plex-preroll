...
type DiscoverClient interface {
	WatchlistItems(ctx context.Context, filter string, params url.Values) (content.Items, error)
	HomeHubs(ctx context.Context, params url.Values) ([]content.Hub, error)
}
...
func (p watchlistProvider) Fetch(ctx context.Context, params map[string]string) (content.Items, error) {
	...
	items, err := p.discover.WatchlistItems(ctx, filter, q)
	if err != nil {
		return nil, err
	}
	return finishDiscover(ctx, p.local, items, params)
}
...
func (p trendingProvider) Fetch(ctx context.Context, params map[string]string) (content.Items, error) {
	hubs, err := p.discover.HomeHubs(ctx, url.Values{})
	if err != nil {
		return nil, err
	}
...
func finishDiscover(ctx context.Context, local Client, items content.Items, params map[string]string) (content.Items, error) {
	libFilter := strings.TrimSpace(params["inLibrary"])
	wantTrailers := isTrue(params["trailers"])

	if libFilter != "" || wantTrailers {
		if err := matchLocal(ctx, local, items); err != nil {
			return nil, err
		}
	}
	if libFilter != "" {
	...
	items = trim(items, params["limit"])
	if wantTrailers {
		return attachTrailers(ctx, local, items)
	}
	return items, nil
}
...
func matchLocal(ctx context.Context, local Client, items content.Items) error {
	for i := range items {
		if items[i].GUID == "" {
			continue
		}
		match, ok, err := local.FindByGUID(ctx, items[i].GUID)
		if err != nil {
			return err
		}
		if ok {
